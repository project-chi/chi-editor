...
class Canvas(QGraphicsScene):
    ...
    def findElement(self, element_type: type):
        if element_type == Molecule:
            element_type = AlphaAtom
        items = self.items()
        if len(items) == 0:
            return ""
        element: type | None = None
        for item in items:
            if isinstance(item, AlphaAtom if element_type == Molecule else element_type):
                element = item
                break
        if element is None:
            return ""
        return Chem.MolToSmiles(mol_from_graphs(element.molecule)) if element_type == AlphaAtom else element.to_string()
